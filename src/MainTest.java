...
public class MainTest {
	...
	public static void main(String[] args) {
		...
		try {
			...
			ResultSet rs = stmt.executeQuery("SELECT reformatted_entity_particpant.participantId, reformatted_entity_particpant.entityId, reformatted_abstract_node.superNodeId, reformatted_entity_particpant.pathwaydbId FROM reformatted_entity_particpant JOIN reformatted_abstract_node ON reformatted_abstract_node.participantId=reformatted_entity_particpant.participantId WHERE reformatted_abstract_node.pathwayDbId='2' AND reformatted_entity_particpant.pathwaydbId='2';");
			SimpleDirectedGraph<Node,Edge> graph = new SimpleDirectedGraph<Node,Edge>(Edge.class);
			while (rs.next()) {
				Node n = new Node(rs.getString(1), rs.getString(2),rs.getString(3),rs.getInt(4));
				graph.addVertex(n);
			}
